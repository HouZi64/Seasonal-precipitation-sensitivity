...
class legacy_effects():
    ...
    def random_forest_co(self,target,features,df,year,indices_name,scenario):
        ...
        data_shap[f'tp_autumn_relative_shap_ratio'] = shap_values[:, 2] / (np.sum(np.abs(shap_values), axis=1))
        data_shap[f'tp_winter_relative_shap_ratio'] = shap_values[:, 3] / (np.sum(np.abs(shap_values), axis=1))
        # data_shap[f'vpd_spring_relative_shap_ratio'] = shap_values[:,4] / (np.sum(np.abs(shap_values),axis=1))
        # data_shap[f'vpd_summer_relative_shap_ratio'] = shap_values[:, 5] / (np.sum(np.abs(shap_values), axis=1))
        # data_shap[f'vpd_autumn_relative_shap_ratio'] = shap_values[:, 6] / (np.sum(np.abs(shap_values), axis=1))
        # data_shap[f'vpd_winter_relative_shap_ratio'] = shap_values[:, 7] / (np.sum(np.abs(shap_values), axis=1))
        # data_shap[f'tm_spring_relative_shap_ratio'] = shap_values[:,8] / (np.sum(np.abs(shap_values),axis=1))
        # data_shap[f'tm_summer_relative_shap_ratio'] = shap_values[:, 9] / (np.sum(np.abs(shap_values), axis=1))
        # data_shap[f'tm_autumn_relative_shap_ratio'] = shap_values[:, 10] / (np.sum(np.abs(shap_values), axis=1))
        # data_shap[f'tm_winter_relative_shap_ratio'] = shap_values[:, 11] / (np.sum(np.abs(shap_values), axis=1))
        ...
        data_shap[f'tp_autumn_shap_value'] = shap_values[:, 2]
        data_shap[f'tp_winter_shap_value'] = shap_values[:, 3]
        # data_shap[f'vpd_spring_shap_value'] = shap_values[:,4] / (np.sum(np.abs(shap_values),axis=1))
        # data_shap[f'vpd_summer_shap_value'] = shap_values[:, 5] / (np.sum(np.abs(shap_values), axis=1))
        # data_shap[f'vpd_autumn_shap_value'] = shap_values[:, 6] / (np.sum(np.abs(shap_values), axis=1))
        # data_shap[f'vpd_winter_shap_value'] = shap_values[:, 7] / (np.sum(np.abs(shap_values), axis=1))
        # data_shap[f'tm_spring_shap_value'] = shap_values[:,8] / (np.sum(np.abs(shap_values),axis=1))
        # data_shap[f'tm_summer_shap_value'] = shap_values[:, 9] / (np.sum(np.abs(shap_values), axis=1))
        # data_shap[f'tm_autumn_shap_value'] = shap_values[:, 10] / (np.sum(np.abs(shap_values), axis=1))
        # data_shap[f'tm_winter_shap_value'] = shap_values[:, 11] / (np.sum(np.abs(shap_values), axis=1))

        # 绘制散点图,保存真值和预测值为csv
        data_shap.to_csv(os.path.join(output_path, f'SHAP_summary_{year}.csv'))
        # 绘制 SHAP summary
        shap.summary_plot(shap_values, X_test[features_new], show=False)
        plt.savefig(os.path.join(output_path, f'Shap_summary_{year}.jpg'), bbox_inches='tight', dpi=300)
        print(f'saved shap summary in ' + os.path.join(output_path, f'Shap_summary_{year}.jpg'))
